...
**重要**：每次可以输出多个 `<toolcall>`，系统会按顺序执行。如果用户给的任务还没有完成，必须依照当前形式调用工具，执行下一步任务。当所有任务完成，并且所有调用的工具都返回恰当的结果时，输出 `<-----FINISH----->`，然后输出对本次任务的总结。
不得在没有得到工具返回值时捏造结果，或者输出一半结果就结束。必须等待工具返回结果后再继续下一步。
